...
export default function Header() {
  ...
  return (
    <div className="w-full">
      <nav
        className="sticky top-0 z-30 block items-start duration-300 ease-in-out"
        style={{
              ...
              <div className="flex w-full items-center justify-between py-[20px]">
                ...
                <div className="items-center mobile-view justify-between lg:flex" id="mobile-nav">
                  ...
                  <div className="flex w-full items-center gap-[10px] lg:gap-[40px] text-black-400">
                      <Link className={`shrink-0 font-medium text-primary ${currentLocation == '/' ? 'active' : ''}`} to="/">
                        {currentLocation == '/' && <i className="bi bi-dot dot-icon"></i>}Home
                      </Link>
                      <Link className={`shrink-0 font-medium text-primary ${currentLocation == '/about-us' ? 'active' : ''}`} to="/about-us">
                      {currentLocation == '/about-us' && <i className="bi bi-dot dot-icon"></i>}
                        Get to Know US
                      </Link>
                      <Link className={`shrink-0 font-medium text-primary ${currentLocation == '/portfolio' ? 'active' : ''}`} to="/portfolio">
                      {currentLocation == '/portfolio' && <i className="bi bi-dot dot-icon"></i>}
                        Portfolio
                      </Link>
                      <Link className={`shrink-0 font-medium text-primary ${currentLocation == '/join-us' ? 'active' : ''}`} to="/join-us">
